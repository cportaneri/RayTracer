...
double * glmModelIntoArray(GLMmodel *model, unsigned int *nb)
{
	unsigned int i, j, k;
	
	*nb = (unsigned int)model->numtriangles;
	double *array = (double *)malloc(model->numtriangles * 9 * sizeof(double));
	
	for(i = 0; i < model->numtriangles; i++){
		for(j = 0; j < 3; j++){
			for(k = 0; k < 3; k++){
				array[9*i + 3*j + k] = model->vertices[3*model->triangles[i].vindices[j] + k];
			}
		}
	}
